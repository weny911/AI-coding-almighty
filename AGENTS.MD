...
- **触发时机**：当用户提出开发需求时，Agent 应主动询问用户是否使用 Spec-Workflow 逻辑。
- **流程选择**：
  - **用户同意使用 Spec-Workflow**：使用 Spec-Workflow MCP 的开发规范，严格执行 TDD（测试驱动开发）思路。
    - 调用 `spec-workflow` 将方案拆解为原子 Task（粒度：单个函数或模块）。
    - 确保每个 Task 都有明确的"验证标准"（即先写测试用例，定义好通过标准，可以选择类似Playwright的工具写测试代码）。
    - 反复自动迭代代码直到验收全部通过。
  - **其他情况**：遵循标准的 R-P-I-V 开发流程规范（见下方）。

### 什么时候用 Spec-Workflow?
- 刚开始聊天，规划要做什么时。询问是否用 Spec-Workflow。
- 觉得自己迷失了，想看当前进度时。
- **指令示例**："用 spec 开发这个新需求"、"查看当前 spec 进度"。
...
- Agent 应识别并执行以下预设指令（用户输入特定指令时自动触发对应操作）：
  - **`sync spec`**：从指定的规范源仓库和分支拉取最新记录，更新以下内容：
    ...
    - 执行前应确认源仓库地址:`https://gitee.com/kaimenshangbao/aicoding.git`和分支名称`master`（询问用户确认）
